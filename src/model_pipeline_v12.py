"""
Model Pipeline V12 (Hierarchical Two-Stage Soft-Cascade)
========================================================
Stage 1: Predicts Outcome (Win/Draw/Loss) natively.
Stage 2: Predicts 36-Class Joint PMF.
Reconciliation: Forces Joint PMF probabilities to align with Stage 1's Outcome 
probabilities via bucketed renormalization.
"""
import pandas as pd
import numpy as np
...
NUM_CLASSES_OUTCOME = 3  # 0: Loss, 1: Draw, 2: Win
NLS_POWER = 1.3
TEMPERATURE = 1.1
...
XGB_PARAMS_JOINT = {
    "objective": "multi:softprob", "num_class": NUM_CLASSES_JOINT, "eval_metric": "mlogloss",
...
# ===========================================================================
# TWO-STAGE CASCADING LOGIC
# ===========================================================================
def soft_cascade_probabilities(prob_out, prob_joint, T=1.1):
    N = len(prob_out)
    
    # Apply Temperature Scaling to Joint PMF first
    ...
    # Calculate implicit outcome marginals from Joint PMF
    sum_joint = np.zeros((N, 3))
    for t in range(MAX_GOALS):
        for o in range(MAX_GOALS):
            c = t * MAX_GOALS + o
            out_idx = np.sign(t - o) + 1  # 0: Loss, 1: Draw, 2: Win
            sum_joint[:, out_idx] += prob_joint[:, c]
            
    # Force alignment: P(Score) = P(Score | Outcome) * P_out(Outcome)
    for t in range(MAX_GOALS):
        for o in range(MAX_GOALS):
            c = t * MAX_GOALS + o
            ...
            p_score_given_out = prob_joint[:, c] / denom
            
            prob_final[:, c] = p_score_given_out * prob_out[:, out_idx]
...
def train_and_predict_subpop(train_df, test_df, feature_cols, T):
    ...
    # Target 1: Outcome (0: Loss, 1: Draw, 2: Win)
    y_out = np.sign(y_t - y_o) + 1
    
    # Target 2: Joint Class (36 classes)
    y_joint = y_t * MAX_GOALS + y_o
    
    # --- STAGE 1: Train Outcome Models ---
    dt_l_out = lgb.Dataset(X_train, y_out, free_raw_data=False)
    lgb_out = lgb.train(LGB_PARAMS_OUT, dt_l_out, num_boost_round=N_ESTIMATORS)
    
    dt_x_out = xgb.DMatrix(X_train, label=y_out)
    xgb_out = xgb.train(XGB_PARAMS_OUT, dt_x_out, num_boost_round=N_ESTIMATORS)
    
    prob_out = (lgb_out.predict(X_test) + xgb_out.predict(xgb.DMatrix(X_test))) / 2.0
    
    # --- STAGE 2: Train Joint PMF Models ---
    dt_l_joint = lgb.Dataset(X_train, y_joint, free_raw_data=False)
    lgb_joint = lgb.train(LGB_PARAMS_JOINT, dt_l_joint, num_boost_round=N_ESTIMATORS)
    
    dt_x_joint = xgb.DMatrix(X_train, label=y_joint)
    xgb_joint = xgb.train(XGB_PARAMS_JOINT, dt_x_joint, num_boost_round=N_ESTIMATORS)
    
    prob_joint = (lgb_joint.predict(X_test) + xgb_joint.predict(xgb.DMatrix(X_test))) / 2.0
    
    # --- RECONCILIATION ---
    prob_final = soft_cascade_probabilities(prob_out, prob_joint, T=T)
...
def main():
    print("=" * 60)
    print("MODEL PIPELINE V12 (Two-Stage Soft-Cascade + Gender-Split)")
    print("=" * 60)
    ...
    # Note: is_home, is_away, is_neutral, tournament_tier should be in test_final.csv now!
    ...
    if gt_path.exists():
        ...
        print("=" * 60)
        print("RESULTS (V12 Two-Stage Soft-Cascade)")
        print("=" * 60)
    ...
    sub.to_csv(DATA_DIR / "submission_v12.csv", index=False)
    print("Saved to submission_v12.csv")
