"""
V23 — V14 + NGBoost Diversity Ensemble
================================================================
Changes vs V14:
  1. ADD NGBoost as alternative base learner alongside LightGBM
  2. Ensemble LGB + NGBoost probabilities for outcome, joint, and regression
  3. Same architecture: Gender-Separate + Transfer M→W + Pseudo-Label + ERM
"""
import pandas as pd
import numpy as np
...
def train_outcome_model(X_train, y_train, X_test, sample_weight=None):
    """Ensemble LGB + NGBoost for 3-class outcome."""
    # LGB
    ...
    ngb_clf.fit(X_train, y_train, X_val=None, Y_val=None,
                sample_weight=w_train)
    prob_ngb = ngb_clf.predict_proba(X_test)
    
    # Ensemble: weighted average (LGB 0.6, NGB 0.4)
...
def train_regressor(X_train, y_train, X_test, sample_weight=None):
    """Ensemble LGB + NGBoost for xG regression."""
    # LGB
    ...
    ngb_reg = NGBRegressor(Dist=Normal, n_estimators=400,
                             learning_rate=0.03, natural_gradient=True,
                             random_state=42, verbose=False)
    ngb_reg.fit(X_train, y_train, X_val=None, Y_val=None,
                sample_weight=w_train)
...
# ===========================================================================
print("="*60)
print("V23 — V14 + NGBoost DIVERSITY ENSEMBLE")
print("="*60)
...
# ===========================================================================
# Stage 1: Outcome (M/S/K) — LGB + NGBoost Ensemble
# ===========================================================================
print("\n--- Stage 1: Outcome (3-class) — LGB+NGBoost Ensemble ---")
...
# ===========================================================================
print("\n--- Stage 2: Joint PMF (36-class) — LGB+NGBoost Ensemble ---")
...
# ===========================================================================
print("\n--- Stage 3: xG Regression — LGB+NGBoost Ensemble ---")
...
out_path = DATA_DIR/"submission_v23.csv"
