"""
Model Pipeline V5 -- Gammafest Masa Kite Lagi
==============================================
Key improvements:
...
# ===========================================================================
LGB_PARAMS = {
    "objective": "poisson", "metric": "poisson", "num_leaves": 50,
...
def erm_predict_batch_dc(lam_team, lam_opp, loss_tensor, rho):
    """ERM with Dixon-Coles bivariate correction."""
    N = len(lam_team)
    M = loss_tensor.shape[0]
    k = np.arange(M)
    lt = np.clip(lam_team, 1e-6, 15.0)
    lo = np.clip(lam_opp, 1e-6, 15.0)

    pmf_t = poisson.pmf(k[None, :], lt[:, None])
    pmf_o = poisson.pmf(k[None, :], lo[:, None])
    pmf_t /= pmf_t.sum(axis=1, keepdims=True)
    pmf_o /= pmf_o.sum(axis=1, keepdims=True)

    # Joint probability with Dixon-Coles correction
...
def timeseries_cv(train_df, feature_cols, loss_tensor, n_folds=3):
    ...
    for fold_i, (tr_start, tr_end, val_start, val_end) in enumerate(cutoffs):
        ...
        y_tr_t = train_df.loc[tr_mask, "team_goals"].values
        y_tr_o = train_df.loc[tr_mask, "opp_goals"].values
        ...
        # Predict
        lam_t = (lgb_t.predict(X_val) + xgb_t.predict(xgb.DMatrix(X_val))) / 2.0
        lam_o = (lgb_o.predict(X_val) + xgb_o.predict(xgb.DMatrix(X_val))) / 2.0
        
        # Fit rho on this fold's validation
        val_df_sub = train_df.loc[val_mask].copy()
        rho = fit_rho(val_df_sub, lam_t, lam_o, loss_tensor)
        
        # ERM with Dixon-Coles
        pt, po = erm_predict_batch_dc(lam_t, lam_o, loss_tensor, rho)
...
def main():
    ...
    t0 = time.time()
    dt_l = lgb.Dataset(X_train, train["team_goals"].values, weight=w_train, free_raw_data=False)
    lgb_t = lgb.train(LGB_PARAMS, dt_l, num_boost_round=N_ESTIMATORS)
    dt_x = xgb.DMatrix(X_train, label=train["team_goals"].values, weight=w_train)
    xgb_t = xgb.train(XGB_PARAMS, dt_x, num_boost_round=N_ESTIMATORS)
    
    do_l = lgb.Dataset(X_train, train["opp_goals"].values, weight=w_train, free_raw_data=False)
    lgb_o = lgb.train(LGB_PARAMS, do_l, num_boost_round=N_ESTIMATORS)
    do_x = xgb.DMatrix(X_train, label=train["opp_goals"].values, weight=w_train)
    xgb_o = xgb.train(XGB_PARAMS, do_x, num_boost_round=N_ESTIMATORS)
    print(f"    Training time: {time.time()-t0:.1f}s")
    
    # Predict lambdas
    print("\n[5] Predicting lambdas...")
    lam_team = (lgb_t.predict(X_test) + xgb_t.predict(xgb.DMatrix(X_test))) / 2.0
    lam_opp  = (lgb_o.predict(X_test) + xgb_o.predict(xgb.DMatrix(X_test))) / 2.0
    
    print(f"    Lambda team: mean={lam_team.mean():.3f}, std={lam_team.std():.3f}")
    print(f"    Lambda opp:  mean={lam_opp.mean():.3f}, std={lam_opp.std():.3f}")
    ...
    np.savez(DATA_DIR / "lambdas_gbm.npz",
             lam_team=lam_team, lam_opp=lam_opp, rho=np.array([rho]))
    print(f"    Lambdas saved to lambdas_gbm.npz")
    
    # ERM with Dixon-Coles
    print("\n[7] ERM prediction with Dixon-Coles...")
    pred_t, pred_o = erm_predict_batch_dc(lam_team, lam_opp, loss_tensor, rho)
    
    # Save submission
    print("\n[8] Generating submission_v5.csv...")
    ...
    sub.to_csv(DATA_DIR / "submission_v5.csv", index=False)
