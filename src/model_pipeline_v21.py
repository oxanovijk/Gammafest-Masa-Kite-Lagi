"""
V21 — V14 + Women Augmentation (Bootstrap 3×) + ZINB
=====================================================
Changes vs V14:
  1. Women Data Augmentation: Bootstrap 3× + Gaussian noise (8806→26418)
  2. ZINB (Zero-Inflated Negative Binomial) for Women xG regression
  3. Men model unchanged from V14
  4. Pseudo-labeling kept from V14
"""
import pandas as pd
import numpy as np
...
def fit_zinb_params(goals):
    ...
    Returns (pi, mu, alpha) for Zero-Inflated Negative Binomial.
    """
    ...
    # If variance <= mean, use Poisson-like (alpha ~ 0)
    if var_g <= mean_g + 0.01:
        # NB with very low alpha, pi = excess zeros beyond Poisson
        pi_est = max(0, p0_emp - np.exp(-mean_g))
        return pi_est, mean_g, 0.001
    
    # NB MoM: fit NB(1/alpha, 1/(1+alpha*mu)) to non-zero data
    # For NB: mean = mu, var = mu + alpha*mu^2
    # Approximate: fit NB to all data first, then estimate pi from excess zeros
    
    # Step 1: Fit plain NB by MoM
    alpha_nb = max(0.001, (var_g - mean_g) / max(mean_g**2, 0.01))
    r_nb = 1.0 / alpha_nb
    p_nb = 1.0 / (1.0 + alpha_nb * mean_g)
    # Expected P(0) from NB
    p0_nb = nbinom.pmf(0, r_nb, p_nb)
    
    # Step 2: Estimate pi from excess zeros
    pi_est = max(0, min(0.5, (p0_emp - p0_nb) / max(1 - p0_nb, 0.001)))
    
    # Step 3: Adjust mu (ZINB mean = (1-pi)*mu)
...
def xg_to_discrete_poisson(xg_t, xg_o, prob_out):
    """Standard Poisson-based discrete prediction (for Men)."""
...
def xg_to_discrete_zinb(xg_t, xg_o, prob_out, pi_t, mu_t, alpha_t, pi_o, mu_o, alpha_o):
    """ZINB-based discrete prediction (for Women)."""
    N = len(xg_t); M = MAX_GOALS
    pred_t = np.zeros(N, dtype=int)
    pred_o = np.zeros(N, dtype=int)
    for i in range(N):
        # Scale mean from xG regression with ZINB params
...
def augment_women(train_w, feature_cols, multiplier=3, noise_std=0.01):
    """Bootstrap augmentation for women data with Gaussian noise."""
...
# ===========================================================================
print("="*60)
print("V21 — V14 + WOMEN AUGMENTATION + ZINB")
print("="*60)
...
# ===========================================================================
# PSEUDO-LABELING (from V14)
# ===========================================================================
print("\n--- Pseudo-Labeling (V14-style) ---")
...
# ===========================================================================
# WOMEN DATA AUGMENTATION (V21 NEW)
# ===========================================================================
# Augment women training data (bootstrap 3× + noise)
...
# ===========================================================================
# ZINB PARAMETER ESTIMATION (V21 NEW)
# ===========================================================================
print("\n--- ZINB Parameter Estimation ---")
# Estimate ZINB params from original training data (not augmented, to keep labels clean)
...
# ===========================================================================
# Stage 1: Outcome (M/S/K)
# ===========================================================================
print("\n--- Stage 1: Outcome (3-class) — Identity V14 + Women Aug ---")
...
# ===========================================================================
print("\n--- Stage 2: Joint PMF (36-class) — Identity V14 + Women Aug ---")
...
# Men regression (Poisson, unchanged)
...
# Also compute women with Poisson for comparison
...
out_path = DATA_DIR/"submission_v21.csv"
