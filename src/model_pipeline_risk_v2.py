"""
Risk-minimizing static pipeline v2 -- Gammafest Masa Kite Lagi.
...
Outputs:
  dataset/submission_risk_v2.csv
  dataset/submission_risk_v2_config.json
  dataset/submission_risk_v2_validation_report.txt
"""
...
OUTPUT_SUB = DATA_DIR / "submission_risk_v2.csv"
OUTPUT_CONFIG = DATA_DIR / "submission_risk_v2_config.json"
OUTPUT_REPORT = DATA_DIR / "submission_risk_v2_validation_report.txt"
...
N_ROUNDS_VAL = 150 if FAST_MODE else 420
N_ROUNDS_FINAL = 300 if FAST_MODE else 700

MAX_GOALS_CANDIDATES = [7, 8, 10]
TEAM_SCALES = [0.92, 0.96, 1.00]
OPP_SCALES = [0.92, 0.96, 1.00]
TEAM_BIASES = [-0.04, 0.00, 0.04]
OPP_BIASES = [-0.04, 0.00, 0.04]
TOTAL_LAMBDA_SCALES = [0.90, 0.92, 0.94, 0.96, 0.98, 1.00]
DRAW_BOOSTS = [1.00, 1.08, 1.16]
LOW_DRAW_BOOSTS = [1.00, 1.12]
LOW_DECISIVE_BOOSTS = [1.00, 1.10]
COMMON_SCORE_BOOSTS = [1.00, 1.06]
HIGH_MARGIN_DAMPENERS = [0.90, 1.00]
TAIL_DAMPENERS = [0.90, 0.95, 1.00]
OUTCOME_ALPHAS = [0.00, 0.06, 0.10, 0.15, 0.20]
OUTCOME_THRESHOLDS = [0.0, 0.48, 0.56]
OUTCOME_PENALTY_ALPHAS = [0.00, 0.10]
BALANCED_DRAW_BOOSTS = [1.00, 1.16]
BALANCED_TOTAL_SCALES = [0.88, 1.00]
NEUTRAL_TOTAL_SCALES = [0.94, 1.00]
...
def predict_scores(lambda_team, lambda_opp, config: CandidateConfig, outcome_probs=None, df: pd.DataFrame | None = None):
...
def aggregate_artifact_predictions(config: CandidateConfig, artifact: dict):
    ...
    pred_t, pred_o = predict_scores(lambda_team, lambda_opp, config, outcome_probs=outcome_probs, df=artifact["val"])
...
def evaluate_candidate(config: CandidateConfig, artifacts: list[dict], baseline: CandidateResult | None, thresholds: dict) -> CandidateResult:
    ...
    fold_instability = 0.20 * float(np.std(fold_awmae)) + 0.10 * max(0.0, float(latest_fold_awmae - np.mean(fold_awmae) - 0.06))
    outcome_drop = 3.0 * max(0.0, base["outcome_accuracy"] - metrics["outcome_accuracy"] - 0.002)
    exact_drop = 1.5 * max(0.0, base["exact_accuracy"] - metrics["exact_accuracy"] - 0.004)
    avg_total_penalty = 0.60 * max(0.0, dist["avg_total_goals"] - 2.70) ** 2 + 0.30 * max(0.0, 2.50 - dist["avg_total_goals"]) ** 2
    high_tail = 0.75 * max(0.0, dist["score_ge5_share"] - thresholds["tail_threshold"]) ** 2
    balanced = segments.get("balanced_elo", {})
    neutral = segments.get("neutral", {})
    non_neutral = segments.get("non_neutral", {})
    balanced_penalty = 0.0
    if balanced:
        balanced_penalty = 1.5 * max(0.0, 0.50 - balanced["outcome_accuracy"]) ** 2 + 0.4 * max(
            0.0, balanced["weighted_awmae"] - metrics["weighted_awmae_power_1_5"] - 0.35
        )
    neutral_penalty = 0.0
    if neutral and non_neutral:
        neutral_penalty = 0.6 * max(0.0, neutral["weighted_awmae"] - non_neutral["weighted_awmae"] - 0.25)
    collapse = 0.05 * max(0.0, dist["top3_score_share"] - thresholds["top3_threshold"]) ** 2
    yearly_penalty = yearly_distribution_penalty(yearly, dist["draw_share"])
    if not config.risk_penalties_enabled:
        avg_total_penalty = high_tail = collapse = balanced_penalty = neutral_penalty = yearly_penalty = 0.0
    ...
    risk_components = {
        ...
        "avg_total_goals_penalty": float(avg_total_penalty),
...
def ablation_accepts(prev: CandidateResult, cand: CandidateResult) -> tuple[bool, str]:
    ...
    collapse_ok = cand.distribution["top3_score_share"] <= 0.72
    avg_total_ok = cand.distribution["avg_total_goals"] <= 3.00
    ...
    ok = (aw_ok and outcome_ok and exact_ok and instability_ok and collapse_ok and avg_total_ok) or clear_aw_win
    reasons = []
    if not aw_ok:
    ...
    if not avg_total_ok:
        reasons.append("avg_total_too_high")
...
def tune_candidates(artifacts: list[dict], model_names: list[str]) -> tuple[CandidateResult, list[dict], dict]:
    ...
    def build_stage_configs(stage_name: str, base_config: CandidateConfig) -> list[CandidateConfig]:
        ...
        if stage_name == "08_risk_objective_v2":
            return [
                CandidateConfig(
                    "08_risk_objective_v2",
                    "risk_penalties_on",
...
def fit_final_predictions(train: pd.DataFrame, test: pd.DataFrame, feature_cols: list[str], chosen: CandidateResult, model_names: list[str]):
    ...
    pred_t, pred_o = predict_scores(lambda_team, lambda_opp, chosen.config, outcome_probs=outcome_probs, df=test)
...
def acceptance_decision(local15, local13, friend15, friend13, final_dist, thresholds):
    ...
    if checks["target_power_1_3_reached"] is False or checks["target_outcome_reached"] is False or checks["target_exact_reached"] is False:
        return "TARGET_NOT_REACHED", checks
    ...
    if not (checks["tail_guard"] and checks["avg_total_guard"]):
        return "NOT_ACCEPTED", checks
    return "ACCEPTED_RISK_V2", checks
...
def write_outputs(
    ...
    config = {
        ...
        "seed": SEED,
        "pipeline": "model_pipeline_risk_v2",
    ...
    lines = []
    lines.append("Risk V2 Static Pipeline Validation Report")
    lines.append("=" * 48)
...
def main():
    t0 = time.time()
    print("=" * 72)
    print("MODEL PIPELINE RISK V2 - STATIC RISK MINIMIZATION")
    print("=" * 72)
