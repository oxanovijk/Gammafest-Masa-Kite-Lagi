"""
Risk-minimizing static pipeline v5 outcome experts -- Gammafest Masa Kite Lagi.
...
Outputs:
  dataset/submission_risk_v5_outcome_experts.csv
  dataset/submission_risk_v5_outcome_experts_config.json
  dataset/submission_risk_v5_outcome_experts_validation_report.txt
"""
...
OUTPUT_SUB = DATA_DIR / "submission_risk_v5_outcome_experts.csv"
OUTPUT_CONFIG = DATA_DIR / "submission_risk_v5_outcome_experts_config.json"
OUTPUT_REPORT = DATA_DIR / "submission_risk_v5_outcome_experts_validation_report.txt"
CACHE_DIR = DATA_DIR / "risk_v5_outcome_experts_cache"
...
OUTCOME_TEMPERATURES = [0.8, 1.0]
EMPIRICAL_PRIOR_BETAS = [0.00, 0.10, 0.20, 0.30]
TOTAL_GOAL_DELTAS = [0.00, 0.10, 0.20]
GOAL_DIFF_ETAS = [0.00, 0.10, 0.20]
TAU_TOTALS = [1.2]
TAU_GDS = [1.2]
SEGMENT_TRUST_CAPS = [0.0, 0.25, 0.40]
WOMEN_TOTAL_SCALES = [1.00, 1.12]
MEN_TOTAL_SCALES = [1.00, 1.06]
WOMEN_DRAW_BOOSTS = [0.90, 1.00]
MEN_DRAW_BOOSTS = [1.00]
STALENESS_TRUST_BOOSTS = [0.00, 0.10]
...
CACHE_SCHEMA_VERSION = "risk_v5_outcome_experts_v1"
...
class CandidateConfig:
    ...
    segment_trust_cap: float = 0.0
    women_total_scale: float = 1.0
    men_total_scale: float = 1.0
    women_draw_boost: float = 1.0
    men_draw_boost: float = 1.0
    staleness_trust_boost: float = 0.0
...
def temperature_scale_probs(probs, temperature: float) -> np.ndarray:
...
def segment_expert_trust(df: pd.DataFrame, config: CandidateConfig) -> np.ndarray:
    if df is None or config.segment_trust_cap <= 0:
        return np.zeros(0, dtype=float)
    is_women = df.get("is_women_match", pd.Series(0, index=df.index)).fillna(0).astype(float).values
    staleness = df.get("frozen_strength_staleness", pd.Series(0, index=df.index)).fillna(0).astype(float).values
    trust_team = df.get("prior_trust_team", pd.Series(0.0, index=df.index)).fillna(0).astype(float).values
    trust_opp = df.get("prior_trust_opp", pd.Series(0.0, index=df.index)).fillna(0).astype(float).values
    prior_trust = np.clip((trust_team + trust_opp) / 2.0, 0.0, 1.0)
    staleness_component = np.clip(staleness / 15.0, 0.0, 1.0) * config.staleness_trust_boost
    gender_component = np.where(is_women > 0.5, 1.0, 0.65)
    trust = config.segment_trust_cap * gender_component * np.clip(0.35 + 0.45 * prior_trust + staleness_component, 0.0, 1.0)
    return np.clip(trust, 0.0, config.segment_trust_cap)


def apply_segment_expert_mixture(
    prob: np.ndarray,
    lambda_team: np.ndarray,
    lambda_opp: np.ndarray,
    config: CandidateConfig,
    ...
    if df is None or config.segment_trust_cap <= 0:
        return prob
    trust = segment_expert_trust(df, config)
    if len(trust) == 0 or np.max(trust) <= 0:
        return prob
    is_women = df.get("is_women_match", pd.Series(0, index=df.index)).fillna(0).astype(bool).values
    total_scale = np.where(is_women, config.women_total_scale, config.men_total_scale)
    draw_boost = np.where(is_women, config.women_draw_boost, config.men_draw_boost)
    expert_prob = np.empty_like(prob)
    for gender_value in [False, True]:
        mask = is_women == gender_value
        if not np.any(mask):
            continue
        expert_prob[mask] = score_probability_matrix(
            lambda_team[mask] * total_scale[mask],
            lambda_opp[mask] * total_scale[mask],
            config.max_goals,
            draw_boost=float(draw_boost[mask][0]),
            low_draw_boost=config.low_draw_boost,
            low_decisive_boost=config.low_decisive_boost,
            tail_dampener=config.tail_dampener,
            common_score_boost=config.common_score_boost,
            high_margin_dampener=config.high_margin_dampener,
            outcome_probs=None if outcome_probs is None else outcome_probs[mask],
            outcome_alpha=config.outcome_alpha,
        )
    mixed = (1.0 - trust[:, None, None]) * prob + trust[:, None, None] * expert_prob
    mixed /= np.maximum(mixed.sum(axis=(1, 2), keepdims=True), 1e-12)
    return mixed
...
def predict_scores(
    ...
    if outcome_probs is not None:
        outcome_probs = temperature_scale_probs(outcome_probs, config.outcome_temperature)
    ...
    prob = apply_segment_expert_mixture(prob, lambda_team, lambda_opp, config, outcome_probs=outcome_probs, df=df)
...
def validation_segment_snapshot(pred_t, pred_o, true_t, true_o, weights, df: pd.DataFrame) -> dict:
    out = {}
    masks = {
        "men": df["is_women_match"].fillna(0).astype(int).eq(0).values
        if "is_women_match" in df.columns
        else np.ones(len(df), dtype=bool),
        "women": df["is_women_match"].fillna(0).astype(int).eq(1).values
        if "is_women_match" in df.columns
        ...
        "high_staleness": df["frozen_strength_staleness"].fillna(0).ge(5).values
        if "frozen_strength_staleness" in df.columns
        else np.zeros(len(df), dtype=bool),
    }
...
def evaluate_candidate(
    ...
    outcome_drop = 3.0 * max(0.0, base["outcome_accuracy"] - metrics["outcome_accuracy"] - 0.003)
    gd_drop = 1.5 * max(0.0, base["goal_diff_accuracy"] - metrics["goal_diff_accuracy"] - 0.004)
    exact_drop = 1.0 * max(0.0, base["exact_accuracy"] - metrics["exact_accuracy"] - 0.004)
    distribution_guard = (
    ...
    women = segments.get("women", {})
    men = segments.get("men", {})
    high_staleness = segments.get("high_staleness", {})
    ...
    women_penalty = 0.0
    if women and women.get("rows", 0) >= 300:
        women_penalty = max(0.0, women["weighted_awmae"] - metrics["weighted_awmae_power_1_5"] - 0.25)
        women_penalty += 2.0 * max(0.0, metrics["outcome_accuracy"] - women["outcome_accuracy"] - 0.03)
    men_outcome_penalty = 0.0
    if men and men.get("rows", 0) >= 1000:
        men_outcome_penalty = 2.0 * max(0.0, metrics["outcome_accuracy"] - men["outcome_accuracy"] - 0.02)
    staleness_penalty = 0.0
    if high_staleness and high_staleness.get("rows", 0) >= 300:
        staleness_penalty = max(0.0, high_staleness["weighted_awmae"] - metrics["weighted_awmae_power_1_5"] - 0.20)
    collapse = 0.0
    yearly_penalty = yearly_distribution_penalty(yearly, dist["draw_share"])
    if not config.risk_penalties_enabled:
        distribution_guard = collapse = balanced_penalty = neutral_penalty = women_penalty = men_outcome_penalty = staleness_penalty = yearly_penalty = outcome_drop = gd_drop = 0.0

    risk_components = {
        "fold_instability_penalty": float(fold_instability),
        "outcome_gap_penalty": float(outcome_drop),
        "goal_diff_drop_penalty": float(gd_drop),
        "exact_drop_penalty": float(exact_drop),
        "women_segment_penalty": float(women_penalty),
        "men_outcome_penalty": float(men_outcome_penalty),
        "staleness_penalty": float(staleness_penalty),
    ...
    selection_score = (
        metrics["weighted_awmae_power_1_5"]
...
def conditional_score_candidate_configs(base_config: CandidateConfig) -> list[CandidateConfig]:
    ...
    variants = [
        replace(base, stage="11_conditional_score_reranker", name="base_erm"),
        replace(
            base,
            stage="11_conditional_score_reranker",
            name="outcome_soft",
        ...
        replace(
            ...
            outcome_alpha=max(base.outcome_alpha, 0.10),
            total_goal_delta=max(base.total_goal_delta, 0.20),
            segment_trust_cap=max(base.segment_trust_cap, 0.40),
            women_total_scale=max(base.women_total_scale, 1.16),
            women_draw_boost=min(base.women_draw_boost, 0.90),
            staleness_trust_boost=max(base.staleness_trust_boost, 0.10),
        ),
        replace(
            base,
            stage="11_conditional_score_reranker",
            ...
            segment_trust_cap=max(base.segment_trust_cap, 0.25),
            men_total_scale=max(base.men_total_scale, 1.04),
            women_total_scale=max(base.women_total_scale, 1.08),
        ),
        replace(
            base,
            stage="11_conditional_score_reranker",
            name="men_outcome_total",
            outcome_alpha=max(base.outcome_alpha, 0.20),
            outcome_threshold=0.38,
            outcome_penalty_alpha=max(base.outcome_penalty_alpha, 0.15),
            segment_trust_cap=max(base.segment_trust_cap, 0.25),
            men_total_scale=max(base.men_total_scale, 1.08),
            men_draw_boost=max(base.men_draw_boost, 1.06),
        ),
    ]
    seen = set()
    out = []
    for cfg in variants:
        key = (
            ...
            cfg.segment_trust_cap,
            cfg.women_total_scale,
            cfg.men_total_scale,
            cfg.women_draw_boost,
            cfg.men_draw_boost,
            cfg.staleness_trust_boost,
        )
        if key not in seen:
            seen.add(key)
            out.append(cfg)
...
def evaluate_score_reranker_candidate(
    ...
    cfg = replace(base_config, stage="11_conditional_score_reranker", name="oof_loss_reranker", use_score_reranker=True)
...
def ablation_accepts(prev: CandidateResult, cand: CandidateResult) -> tuple[bool, str]:
    ...
    gd_ok = cand.metrics["goal_diff_accuracy"] >= prev.metrics["goal_diff_accuracy"] - 0.005
    ...
    clear_aw_win = aw_delta >= 0.003 and exact_ok and gd_ok and cand.distribution["avg_total_goals"] <= 3.10
    outcome_guard_win = (
        cand.config.stage in {"05_outcome_first_erm", "10_men_women_expert_mixture", "11_conditional_score_reranker"}
    ...
    ok = (
        (aw_ok and outcome_ok and exact_ok and gd_ok and instability_ok and collapse_ok and avg_total_ok and draw_ok)
...
def tune_candidates(artifacts: list[dict], model_names: list[str], profile_name: str) -> tuple[CandidateResult, list[dict], dict]:
    ...
    stage_names = [
        ...
        "09_risk_objective_v5",
        "10_men_women_expert_mixture",
        "11_conditional_score_reranker",
    ]
    ...
    def build_stage_configs(stage_name: str, base_config: CandidateConfig) -> list[CandidateConfig]:
        ...
        if stage_name == "09_risk_objective_v5":
            return [
                CandidateConfig(
                    "09_risk_objective_v5",
                    "risk_penalties_on",
        ...
        if stage_name == "10_men_women_expert_mixture":
            return [
                replace(
                    base_config,
                    stage="10_men_women_expert_mixture",
                    name=f"trust{trust}_ws{ws}_ms{ms}_wd{wd}_md{md}_st{st}_temp{temp}",
                    segment_trust_cap=trust,
                    women_total_scale=ws,
                    men_total_scale=ms,
                    women_draw_boost=wd,
                    men_draw_boost=md,
                    staleness_trust_boost=st,
                    outcome_temperature=temp,
                )
                for trust in SEGMENT_TRUST_CAPS
                for ws in WOMEN_TOTAL_SCALES
                for ms in MEN_TOTAL_SCALES
                for wd in WOMEN_DRAW_BOOSTS
                for md in MEN_DRAW_BOOSTS
                for st in STALENESS_TRUST_BOOSTS
                for temp in OUTCOME_TEMPERATURES
            ]
        if stage_name == "11_conditional_score_reranker":
            return [
                replace(
                    base_config,
                    stage="11_conditional_score_reranker",
...
def acceptance_decision(local15, local13, friend15, friend13, final_dist, thresholds):
    ...
    if not checks["tail_guard"]:
        return "NOT_ACCEPTED", checks
    return "ACCEPTED_RISK_V5_OUTCOME_EXPERTS", checks
...
def write_outputs(
    ...
    config = {
        ...
        "seed": SEED,
        "pipeline": "model_pipeline_risk_v5_outcome_experts",
    ...
    lines = []
    lines.append("Risk V5 Outcome Experts Pipeline Validation Report")
    lines.append("=" * 48)
    ...
    if decision != "ACCEPTED_RISK_V5_OUTCOME_EXPERTS":
        lines.append("")
...
def main():
    t0 = time.time()
    print("=" * 72)
    print("MODEL PIPELINE RISK V5 OUTCOME EXPERTS - GENDER AWARE")
    print("=" * 72)
