"""
Leakage-safe era-aware dynamic-state pipeline v2.

This file is intentionally standalone:
  * it has no dependency on model_pipeline_v5.py;
...
V2 design intent:
  * fix V1's conservative low-total / high-draw bias;
  * make validation selection more sensitive to late-train drift;
  * guard outcome and goal-difference first because AW-MAE punishes them hard;
  * include an internal flaw-audit loop that must pass with no major/medium
    unresolved design flaws before final inference.

Outputs:
  dataset/submission_dynamic_state_v2.csv
  dataset/submission_dynamic_state_v2_config.json
  dataset/submission_dynamic_state_v2_validation_report.txt
  dataset/submission_dynamic_state_v2_candidate_lock.json
"""
...
# Paths and constants
# ============================================================================
PIPELINE_VERSION = "dynamic_state_v2"
SEED = 42
PRIMARY_POWER = 1.5
SECONDARY_POWER = 1.3
...
OUTPUT_SUB = DATA_DIR / "submission_dynamic_state_v2.csv"
OUTPUT_CONFIG = DATA_DIR / "submission_dynamic_state_v2_config.json"
OUTPUT_REPORT = DATA_DIR / "submission_dynamic_state_v2_validation_report.txt"
OUTPUT_LOCK = DATA_DIR / "submission_dynamic_state_v2_candidate_lock.json"
CACHE_DIR = DATA_DIR / "dynamic_state_v2_cache"
...
PRIMARY_FOLDS = [
    ...
    {"name": "F3_2009", "train_end_year": 2008, "valid_start_year": 2009, "valid_end_year": 2009, "weight": 0.22},
    {"name": "F4_2010", "train_end_year": 2009, "valid_start_year": 2010, "valid_end_year": 2010, "weight": 0.25},
    {"name": "F5_2011", "train_end_year": 2010, "valid_start_year": 2011, "valid_end_year": 2011, "weight": 0.33},
]
...
DERIVED_NUMERIC_FEATURES = [
    ...
    "is_major_tournament",
    "is_elite_tournament",
    "tournament_weight_x_forecast_years",
    "women_x_year_since_2011",
    "women_x_forecast_years",
    "women_x_post_2010",
    "women_x_tournament_weight",
    "friendly_x_year_since_2011",
    "friendly_x_forecast_years",
    "major_x_year_since_2011",
    "major_x_forecast_years",
]
...
class CandidateConfig:
    ...
    tau_gd: float = 1.20
    total_lift_per_forecast_year: float = 0.000
    women_lift_per_forecast_year: float = 0.000
    max_total_lift: float = 1.12
    draw_cell_weight: float = 1.00
    outcome_sharpness: float = 1.00
    gd_signal_scale: float = 1.00
...
def merge_final_with_raw(final_df: pd.DataFrame, raw_df: pd.DataFrame, is_train: bool) -> pd.DataFrame:
    ...
    out["forecast_years_since_state"] = 0.0
    out["post_2008"] = (out["year"] >= 2009).astype(float)
    out["post_2010"] = (out["year"] >= 2011).astype(float)
    ...
    out["tournament_weight_x_forecast_years"] = out["tournament_weight"] * out["forecast_years_since_state"]
    out["women_x_year_since_2011"] = out["is_women_match"] * out["year_since_2011"]
    out["women_x_forecast_years"] = out["is_women_match"] * out["forecast_years_since_state"]
    out["women_x_post_2010"] = out["is_women_match"] * out["post_2010"]
    ...
    out["friendly_x_forecast_years"] = out["is_friendly"] * out["forecast_years_since_state"]
    out["major_x_year_since_2011"] = out["is_major_tournament"] * out["year_since_2011"]
    out["major_x_forecast_years"] = out["is_major_tournament"] * out["forecast_years_since_state"]
    if is_train:
...
def set_forecast_horizon_features(df: pd.DataFrame, anchor_year: int) -> pd.DataFrame:
    """Set metadata-only forward-horizon features for validation/test walks."""
    out = df.copy()
    horizon = np.maximum(0.0, out["year"].astype(float).values - float(anchor_year))
    out["forecast_years_since_state"] = horizon
    out["tournament_weight_x_forecast_years"] = out["tournament_weight"].astype(float).values * horizon
    out["women_x_forecast_years"] = out["is_women_match"].astype(float).values * horizon
    out["friendly_x_forecast_years"] = out["is_friendly"].astype(float).values * horizon
    out["major_x_forecast_years"] = out["is_major_tournament"].astype(float).values * horizon
    return out
...
def matrix_from_heads(
    ...
    row_context: pd.Series | dict[str, Any] | None = None,
) -> np.ndarray:
    max_goals = config.max_goals
    size = max_goals + 1
    row_context = {} if row_context is None else row_context
    horizon = safe_float(row_context.get("forecast_years_since_state", 0.0), 0.0)
    is_women = safe_float(row_context.get("is_women_match", 0.0), 0.0)
    total_lift = 1.0 + config.total_lift_per_forecast_year * min(horizon, 15.0)
    total_lift += config.women_lift_per_forecast_year * is_women * min(horizon, 15.0)
    total_lift = float(np.clip(total_lift, 0.90, config.max_total_lift))
    lambda_team *= total_lift
    lambda_opp *= total_lift
    total_pred *= total_lift
    gd_pred *= config.gd_signal_scale
    ...
    if abs(config.outcome_sharpness - 1.0) > 1e-9:
        outcome_probs = np.power(outcome_probs, config.outcome_sharpness)
        outcome_probs = np.maximum(outcome_probs, 1e-8)
        outcome_probs = outcome_probs / outcome_probs.sum()
    ...
    for a in range(size):
        for b in range(size):
            ...
            if a == b:
                matrix[a, b] *= config.draw_cell_weight
...
def evaluate_candidate_on_folds(
    ...
    for fold in folds:
        ...
        fold_train_p = set_forecast_horizon_features(add_prior_features(fold_train, priors), int(fold["train_end_year"]))
        fold_valid_p = set_forecast_horizon_features(add_prior_features(fold_valid, priors), int(fold["train_end_year"]))
...
def compute_selection_components(
    ...
    acceptance = {
        ...
        "primary_accept": bool(
            ...
            and f4_worse <= 0.008
            and distribution.get("score_ge5", 0.0) <= 0.035
            and config.pair_consistency
            and bool(pair_diagnostics.get("pair_consistency_pass", True))
            and (fold_improvements >= 4 or weighted_improvement >= 0.012)
        ),
    }
...
def build_candidate_registry(fast_mode: bool) -> tuple[list[CandidateConfig], list[dict[str, Any]]]:
    ...
    for track in active_tracks:
        for smoothing in SMOOTHING_CANDIDATES:
            registry.append(
                CandidateConfig(
                    name=f"{track}_s{int(smoothing)}_era_total_gd",
                    track=track,
                    base_lr=TRACK_LR[track],
                    smoothing=smoothing,
                    max_goals=max_goals,
                    total_lift_per_forecast_year=0.004,
                    women_lift_per_forecast_year=0.005,
                    max_total_lift=1.14,
                    draw_cell_weight=0.94,
                    outcome_sharpness=1.08,
                    gd_signal_scale=1.04,
    ...
    registry.append(CandidateConfig(name="slow_s50_draw_lift_mild", track="slow", base_lr=TRACK_LR["slow"], smoothing=50.0, max_goals=max_goals, total_lift_per_forecast_year=0.003, women_lift_per_forecast_year=0.004, max_total_lift=1.10, draw_cell_weight=0.96, outcome_sharpness=1.05, gd_signal_scale=1.02, ablation_group="era_draw_lift_mild"))
    registry.append(CandidateConfig(name="medium_s50_draw_lift_stronger", track="medium", base_lr=TRACK_LR["medium"], smoothing=50.0, max_goals=max_goals, total_lift_per_forecast_year=0.006, women_lift_per_forecast_year=0.008, max_total_lift=1.18, draw_cell_weight=0.90, outcome_sharpness=1.12, gd_signal_scale=1.08, ablation_group="era_draw_lift_stronger"))
    mid = CandidateConfig(name="medium_s50_gender_state_off", track="medium", base_lr=TRACK_LR["medium"], smoothing=50.0, max_goals=max_goals, use_gender_state=False, ablation_group="gender_state_off", **v2_defaults)
    registry.append(mid)
    registry.append(CandidateConfig(name="medium_s50_confidence_gate_off", track="medium", base_lr=TRACK_LR["medium"], smoothing=50.0, max_goals=max_goals, use_confidence_gate=False, ablation_group="confidence_gate_off", **v2_defaults))
    registry.append(CandidateConfig(name="medium_s50_outcome_update_off", track="medium", base_lr=TRACK_LR["medium"], smoothing=50.0, max_goals=max_goals, outcome_aware_update=False, ablation_group="outcome_aware_update_off", **v2_defaults))
    registry.append(CandidateConfig(name="medium_s50_total_state_off", track="medium", base_lr=TRACK_LR["medium"], smoothing=50.0, max_goals=max_goals, total_goals_state=False, ablation_group="total_goals_state_off", **v2_defaults))
    registry.append(CandidateConfig(name="medium_s50_pair_consistency_off", track="medium", base_lr=TRACK_LR["medium"], smoothing=50.0, max_goals=max_goals, pair_consistency=False, ablation_group="pair_consistency_off", **v2_defaults))
    registry.append(CandidateConfig(name="medium_s50_static_fallback_off", track="medium", base_lr=TRACK_LR["medium"], smoothing=50.0, max_goals=max_goals, static_fallback=False, ablation_group="static_fallback_off", **v2_defaults))
    registry.append(CandidateConfig(name="medium_s50_era_total_off", track="medium", base_lr=TRACK_LR["medium"], smoothing=50.0, max_goals=max_goals, draw_cell_weight=0.94, outcome_sharpness=1.08, gd_signal_scale=1.04, ablation_group="era_total_lift_off"))
    registry.append(CandidateConfig(name="medium_s50_draw_dampen_off", track="medium", base_lr=TRACK_LR["medium"], smoothing=50.0, max_goals=max_goals, total_lift_per_forecast_year=0.004, women_lift_per_forecast_year=0.005, max_total_lift=1.14, draw_cell_weight=1.00, outcome_sharpness=1.08, gd_signal_scale=1.04, ablation_group="draw_dampen_off"))
...
def fit_final_and_predict(train: pd.DataFrame, test: pd.DataFrame, sample: pd.DataFrame, config: CandidateConfig, fast_mode: bool) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    train_p = set_forecast_horizon_features(add_prior_features(train, priors), max_train_year)
    test_p = set_forecast_horizon_features(add_prior_features(test, priors), max_train_year)
...
def acceptance_decision(
    ...
    checks = {
        ...
        "score_ge5_guard": final_distribution.get("score_ge5", 1.0) <= 0.035,
    ...
    if local15 is not None and local15.get("available"):
        ...
        checks["gt_score_ge5_guard"] = local15.get("distribution", {}).get("score_ge5", 1.0) <= 0.035
    else:
    ...
    if accepted_core:
        return "ACCEPTED_DYNAMIC_STATE_V2", checks
...
# ============================================================================
# V1 flaw analysis and v2 design audit
# ============================================================================
def previous_model_flaw_analysis() -> dict[str, Any]:
    return {
        "executive_summary": (
            "V1 was leakage-safe and improved validation, but it selected a slow, heavily guarded "
            "dynamic update that underreacted to 2011-2026 drift. The main practical failure mode "
            "was lower outcome/GD quality and a low-total, high-draw prediction distribution."
        ),
        "what_the_model_was_trying_to_optimize": (
            "Minimize validation weighted AW-MAE p1.5 while preserving outcome, exact score, and "
            "goal-difference accuracy under strict leakage constraints."
        ),
        "what_worked": [
            "Candidate selection was validation-only.",
            "Candidate lock was written before GT/friend reads.",
            "Prediction-only validation/test state updates prevented direct label leakage.",
            "Pair consistency prevented two-row match contradictions.",
            "Validation p1.5 improved against the apples-to-apples static baseline.",
        ],
        "main_flaws_in_validation_design": [
            "The latest primary fold was only 2011, while test spans 2011-2026.",
            "The fold design did not separately stress 2009, 2010, and 2011 recency behavior.",
            "Selection over-weighted small AW-MAE gains without enough distribution/outcome drift pressure.",
        ],
        "main_flaws_in_dynamic_state_design": [
            "The selected slow_s50 update was too timid after gates and shrinkage.",
            "Prediction-only updates safely avoided leakage but reinforced low-total/draw bias.",
            "The state key was useful but too coarse for era/gender/tournament drift.",
        ],
        "main_flaws_in_score_matrix_erm_design": [
            "Separate heads could disagree, and the matrix fusion compromised between them.",
            "ERM optimized expected AW-MAE but did not directly control draw share and total-goal drift.",
            "The matrix lacked a validation-selected forecast-horizon correction.",
        ],
        "main_flaws_in_selection_guards": [
            "Outcome/GD were guarded as secondary drops instead of being first-class optimization pressure.",
            "The score_ge5 guard discouraged risk but did not distinguish bad tails from needed total-goal lift.",
            "Stress vetoes made the selected config safe but helped keep it conservative.",
        ],
        "why_validation_improved_but_gt_did_not": (
            "The validation window was old relative to the test window, so the safe small update looked good "
            "in 2003-2011 but underfit later drift. It also traded a slight exact-score gain for outcome/GD "
            "weakness, which AW-MAE punishes heavily."
        ),
        "why_friend_csv_beat_it": (
            "The friend CSV had higher outcome and GD accuracy and a higher average total-goal distribution, "
            "with fewer draws. Those properties matter more to AW-MAE than V1's slight exact-score advantage."
        ),
        "leakage_safety_tradeoffs": [
            "No GT/friend tuning is allowed; post-lock metrics can diagnose but cannot rescue a candidate.",
            "Prediction-only test updates are safe but cannot correct wrong beliefs with actual results.",
            "Metadata-only future-horizon features are allowed because they use dates/gender/tournament only.",
        ],
        "concrete_design_lessons_for_next_pipeline": [
            "Use recency-split validation and future-horizon features so selection sees drift pressure.",
            "Optimize outcome/GD and distribution shape alongside AW-MAE.",
            "Let validation select total-goal lift and draw dampening rather than hard-coding a cautious matrix.",
            "Keep pair consistency, lock discipline, and post-lock reporting from V1.",
        ],
        "prioritized_fixes_for_next_model": [
            "Recency-weighted 2009/2010/2011 primary folds.",
            "Forecast-horizon metadata features for validation/test.",
            "Validation-selected total lift, draw dampening, outcome sharpness, and GD scaling.",
            "Outcome/GD-first selection penalties and a less blunt score_ge5 guard.",
            "Internal design self-audit before final inference.",
        ],
    }


def run_design_self_audit(candidates: list[CandidateConfig]) -> dict[str, Any]:
    findings: list[dict[str, Any]] = []
    dynamic = [c for c in candidates if c.track != "static"]

    def add(severity: str, title: str, status: str, mitigation: str) -> None:
        findings.append({"severity": severity, "title": title, "status": status, "mitigation": mitigation})

    if PIPELINE_VERSION != "dynamic_state_v2":
        add("major", "Wrong pipeline version", "unresolved", "Use v2 output names and cache namespace.")
    else:
        add("info", "Standalone v2 namespace", "resolved", "Outputs/cache/lock use dynamic_state_v2 names.")

    latest_weight = PRIMARY_FOLDS[-1]["weight"]
    if len(PRIMARY_FOLDS) < 5 or latest_weight < 0.30:
        add("medium", "Validation recency pressure too weak", "unresolved", "Use split 2009/2010/2011 folds with latest fold weight >= .30.")
    else:
        add("info", "Validation recency pressure", "resolved", "Primary folds split 2009, 2010, and 2011 with recency-heavy weights.")

    if not dynamic or not any(c.total_lift_per_forecast_year > 0 or c.women_lift_per_forecast_year > 0 for c in dynamic):
        add("medium", "No future total-goal drift control", "unresolved", "Add validation-selected metadata-only total-goal lift.")
    else:
        add("info", "Future total-goal drift control", "resolved", "Dynamic candidates include forecast-horizon total lift and women-specific lift.")

    if not any(c.draw_cell_weight < 1.0 for c in dynamic):
        add("medium", "No draw-bias correction", "unresolved", "Add validation-selected draw-cell dampening.")
    else:
        add("info", "Draw-bias correction", "resolved", "Dynamic candidates include draw-cell dampening to fight high-draw conservatism.")

    if not any(c.gd_signal_scale > 1.0 or c.outcome_sharpness > 1.0 for c in dynamic):
        add("medium", "Outcome/GD not first-class", "unresolved", "Add outcome sharpness/GD signal scaling candidates.")
    else:
        add("info", "Outcome/GD first-class controls", "resolved", "Dynamic candidates expose outcome sharpness and GD scaling.")

    source = Path(__file__).read_text(encoding="utf-8")
    forbidden = ["model_pipeline_v5", "submission_v3", "submission_v4", "submission_v5", "submission_v8_anchor"]
    forbidden_hits = []
    for token in forbidden:
        if re.search(rf"^\s*(from|import)\s+{re.escape(token)}\b", source, flags=re.MULTILINE):
            forbidden_hits.append(token)
    if forbidden_hits:
        add("major", "Forbidden imports/anchors", "unresolved", f"Remove forbidden references: {forbidden_hits}")
    else:
        add("info", "Forbidden anchor/import guard", "resolved", "No forbidden V5 import or old submission anchor import is present.")

    if "write_candidate_lock" not in source or "local_submission_metrics" not in source:
        add("major", "Missing lock/report split", "unresolved", "Keep candidate lock before GT/friend reporting.")
    else:
        add("info", "Candidate lock before reporting", "resolved", "Candidate lock and post-lock reporting remain separate code paths.")

    unresolved = [f for f in findings if f["status"] != "resolved" and f["severity"] in {"major", "medium"}]
    return {
        "iterations": 1,
        "passed_no_major_or_medium": len(unresolved) == 0,
        "unresolved_major_or_medium": unresolved,
        "findings": findings,
        "residual_minor_limitations": [
            "No leakage-safe training label exists after 2011, so 2012-2026 drift can only be approximated using metadata and historical trends.",
            "Friend/GT metrics remain post-lock diagnosis only and are not used for candidate rescue.",
        ],
    }
...
def build_report(
    ...
    lines: list[str] = []
    lines.append("Dynamic State V2 Validation Report")
    lines.append("=" * 40)
    lines.append(f"Decision: {decision}")
    lines.append("")
    lines.append("Previous model flaw analysis")
    lines.append(json.dumps(flaw_analysis, indent=2)[:10000])
    lines.append("")
    lines.append("V2 self-audit")
    lines.append(json.dumps(self_audit, indent=2)[:10000])
    lines.append("")
...
def main() -> None:
    ...
    print(f"[dynamic_state_v2] loading core data from {DATA_DIR}")
    ...
    print(f"[dynamic_state_v2] train rows={len(train)} test rows={len(test)} max_train_date={max_train_date}")

    candidates, skipped_items = build_candidate_registry(fast_mode)
    registry_hash = json_hash([asdict(c) for c in candidates])
    print(f"[dynamic_state_v2] pre-registered candidate registry hash={registry_hash}")
    flaw_analysis = previous_model_flaw_analysis()
    self_audit = run_design_self_audit(candidates)
    if not self_audit["passed_no_major_or_medium"]:
        raise RuntimeError(f"V2 self-audit failed: {self_audit['unresolved_major_or_medium']}")
    ...
    print(f"[dynamic_state_v2] selected={selected_config.name} track={selected_config.track} validation={short_metrics(selected.metrics)}")

    sensitivity = []
    if args.skip_sensitivity:
        sensitivity = [{"skipped": True, "reason": "command_line_skip"}]
    else:
        print("[dynamic_state_v2] running sensitivity diagnostics")
    ...
    validation_payload = {
        ...
        "flaw_analysis_hash": json_hash(flaw_analysis),
        "self_audit_hash": json_hash(self_audit),
    }
    ...
    print(f"[dynamic_state_v2] candidate lock written hash={lock_hash}")

    print("[dynamic_state_v2] fitting final model and writing submission")
    ...
    config_payload = {
        ...
        "previous_model_flaw_analysis": flaw_analysis,
    ...
    print(f"[dynamic_state_v2] wrote {OUTPUT_SUB}")
    print(f"[dynamic_state_v2] wrote {OUTPUT_CONFIG}")
    print(f"[dynamic_state_v2] wrote {OUTPUT_REPORT}")
    print(f"[dynamic_state_v2] decision={decision}")
