"""
Risk-minimizing static pipeline v3 -- Gammafest Masa Kite Lagi.
...
Outputs:
  dataset/submission_risk_v3.csv
  dataset/submission_risk_v3_config.json
  dataset/submission_risk_v3_validation_report.txt
"""
...
OUTPUT_SUB = DATA_DIR / "submission_risk_v3.csv"
OUTPUT_CONFIG = DATA_DIR / "submission_risk_v3_config.json"
OUTPUT_REPORT = DATA_DIR / "submission_risk_v3_validation_report.txt"
CACHE_DIR = DATA_DIR / "risk_v3_cache"
...
N_ROUNDS_VAL = 110 if FAST_MODE else 420
N_ROUNDS_FINAL = 260 if FAST_MODE else 700
...
TOTAL_LAMBDA_SCALES = [0.98, 1.00, 1.02, 1.04]
DRAW_BOOSTS = [1.00]
...
FAST_FEATURE_PROFILES = ["core_only", "core_plus_context_no_te", "all_features_safe"]
...
def build_feature_profiles(train: pd.DataFrame, test: pd.DataFrame, candidate_cols: list[str]) -> tuple[dict, dict, dict]:
    ...
    profiles = {
        "core_only": sorted(set(core)),
        "core_plus_metadata": sorted(set(core + metadata + risk_cols)),
        "core_plus_context_no_te": sorted(set(core + metadata + context_no_te + risk_cols)),
        # Existing KFold target encodings are intentionally excluded in FAST_MODE;
        # time-aware TE is reported as skipped unless explicitly implemented later.
        "core_plus_timeaware_te": sorted(set(core + metadata + context_no_te + risk_cols)),
        "all_features_safe": all_safe,
    }
    if FAST_MODE:
        ...
        for col in numeric_cols:
            if col not in cols:
                if name.endswith("_no_te") and is_existing_te_column(col):
                    ...
                    drop[col] = "excluded_by_core_only_profile"
                elif name == "core_plus_context_no_te" and is_existing_te_column(col):
                    drop[col] = "excluded_existing_target_encoding_for_temporal_safety"
                else:
...
def feature_cache_key(profile_name: str, feature_cols: list[str], train: pd.DataFrame, test_rows: int) -> tuple[str, dict]:
    payload = {
        "pipeline_version": "risk_v3",
...
def build_validation_artifacts(train: pd.DataFrame, feature_cols: list[str], profile_name: str, test_rows: int):
    print(f"\n[2] Building train-only rolling fold artifacts for profile={profile_name}...")
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    cache_key, cache_payload = feature_cache_key(profile_name, feature_cols, train, test_rows)
...
def evaluate_candidate(config: CandidateConfig, artifacts: list[dict], baseline: CandidateResult | None, thresholds: dict) -> CandidateResult:
    ...
    distribution_guard = (
        ...
        + 0.10 * max(0.0, dist["avg_total_goals"] - 2.80) ** 2
        + 1.20 * max(0.0, 2.58 - dist["avg_total_goals"]) ** 2
        + 2.00 * max(0.0, dist["draw_share"] - 0.165) ** 2
    )
...
def ablation_accepts(prev: CandidateResult, cand: CandidateResult) -> tuple[bool, str]:
    ...
    avg_total_ok = 2.50 <= cand.distribution["avg_total_goals"] <= 3.00
    draw_ok = cand.distribution["draw_share"] <= 0.180
...
def acceptance_decision(local15, local13, friend15, friend13, final_dist, thresholds):
    ...
    if not (checks["tail_guard"] and checks["avg_total_guard"]):
        return "NOT_ACCEPTED", checks
    return "ACCEPTED_RISK_V3", checks
...
def write_outputs(
    ...
    config = {
        ...
        "seed": SEED,
        "pipeline": "model_pipeline_risk_v3",
    ...
    lines = []
    lines.append("Risk V3 Static Pipeline Validation Report")
    lines.append("=" * 48)
...
def main():
    t0 = time.time()
    print("=" * 72)
    print("MODEL PIPELINE RISK V3 - DIAGNOSTIC-FIRST STATIC RISK MINIMIZATION")
    print("=" * 72)
    ...
    for profile_name, cols in feature_profiles.items():
        artifacts, model_names, cache_meta = build_validation_artifacts(train, cols, profile_name, len(test))
