"""
V24 — V23 + Multi-Task Shared Encoder (Gender-Agnostic Feature Learning)
========================================================================
Changes vs V23:
  1. ADD Multi-Task Learning: shared encoder trained on Men+Women jointly
  2. Gender-specific heads on top of shared representations
  3. Shared encoder captures universal football dynamics
  4. Gender-aware head captures sex-specific scoring distributions
  
Architecture (conceptual):
  Shared Encoder (LGB on all data) → Gender-specific LGB heads
  Practical: train LGB on ALL data with gender flag as feature,
            then use leaf indices as shared features + gender-specific LGB
  
Implementation:
  Step A: Train a shared LGB on Men+Women combined (with is_woman flag)
  Step B: Extract leaf/prediction features from shared model
  Step C: Train separate LGB+NGBoost per gender on shared features
"""
import pandas as pd
import numpy as np
...
# ===========================================================================
# TRAINING HELPERS — LGB + NGBoost Ensemble (same as V23)
# ===========================================================================
...
# ===========================================================================
# SHARED ENCODER (Multi-Task Learning via Leaf Features)
# ===========================================================================
def build_shared_encoder(train_m, train_w, test_m, test_w, feature_cols):
    """
    Train a shared LGB model on Men+Women combined (with is_woman flag).
    Extract leaf indices as shared feature representation.
    Returns augmented feature matrices with shared encoding.
    """
    ...
    # Shared encoder: predict outcome for both genders
    ...
    # Extract leaf indices as shared features
    ...
    # Convert leaf indices to features (bag-of-leaves / one-hot-like)
    ...
    # Use leaf predictions directly as additional features
    X_sh_train_m = leaf_train_m.astype(np.float32)
    X_sh_train_w = leaf_train_w.astype(np.float32)
    X_sh_test_m = leaf_test_m.astype(np.float32)
    X_sh_test_w = leaf_test_w.astype(np.float32)
    
    return X_sh_train_m, X_sh_train_w, X_sh_test_m, X_sh_test_w

# ===========================================================================
# MAIN
# ===========================================================================
print("="*60)
print("V24 — V23 + MULTI-TASK SHARED ENCODER")
print("="*60)
...
# ===========================================================================
# STEP B: Augment features with shared encoding
# ===========================================================================
...
# Concatenate shared encoding to base features
...
print(f"  Augmented features: {X_m_aug.shape[1]} (was {len(feature_cols)})")
...
# ===========================================================================
# Stage 1: Outcome (M/S/K) — Gender-Separate with Shared Encoder + LGB/NGBoost
# ===========================================================================
print("\n--- Stage 1: Outcome (3-class) — Multi-Task Shared Features ---")

X_m = X_m_aug
X_test_m = X_test_m_aug
...
# Women with transfer learning + shared encoding
X_w = X_w_aug
X_test_w = X_test_w_aug
...
# ===========================================================================
print("\n--- Stage 2: Joint PMF (36-class) — Multi-Task Shared Features ---")
...
# ===========================================================================
print("\n--- Stage 3: xG Regression — Multi-Task Shared Features ---")
...
# ===========================================================================
# Score comparison and final selection
# ===========================================================================
...
out_path = DATA_DIR/"submission_v24.csv"
