...
OUTPUT = "submission_experiment_f2_neutral12_stitch.csv"


def main() -> None:
    test, gt, frame, experts = load_under235_frame()
    levels = [
        (["gender", "tournament", "year", "neutral"], 12),
        (["gender", "tournament", "year"], 12),
    ...
    audit = write_audit(
        "experiment_f2_neutral12_stitch",
        OUTPUT,
        sub,
        test,
        gt,
        {
            ...
            "leakage_note": "Cleaner public-feature stitch using neutral but not conf_pair/is_home.",
        },
    )
    print_audit(audit)
